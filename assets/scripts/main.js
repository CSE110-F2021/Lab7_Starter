...
const router = new Router(function () {
   ...
   */
  let sectionCards = document.querySelector("section--recipe-cards");
  // let sectionExpandCards = document.querySelector("section--recipe-expand");
  sectionCards.classList.add("shown");
  // sectionExpandCards.classList.remove("shown");
});
...
function createRecipeCards() {
  for (let i = 0; i < recipes.length; i++) {
     ...
     */
    let currentPageName = recipeData[recipes[i]]["page-name"];
    let cardRouter = new Router(router.home);
    cardRouter.addPage(currentPageName, function () {
      recipeCard.addEventListener(
        "click",
        bindRecipeCard(recipeCard, currentPageName)
      );
    });
